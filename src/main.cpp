...
#if defined(NDEBUG)
# error "Pissedoffcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcbfc12a4cc3df9b9a01cd6efc81f546b5da5fede8d89b344db3ff49140e4bdca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pissedoffcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pissedoffcoin Signed Message:\n";
...
{
    // Pissedoffcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Pissedoffcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pissedoffcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Pissedoffcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pissedoffcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pissedoffcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x83d8aca233989c8ba6dd1b8c738c02d66ec3995cba18b6f7195eef3b8bd9e0ae");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Face is the mirror of heart as bunny said";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04135831dcee9f20897655e9d55bf52dbeeecbecc8169e666c19d2e094c92dab34156d22a5703cd9f0197723533e9bfefa4d6192076108adddc77cbbd788c65841") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1564423784;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084744680;

        if (fTestNet)
        {
            block.nTime    = 1564423762;
            block.nNonce   = 385932976;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9223eb4cc3c9acdb2514e6cccc78ebe5857e5dabb501c0cfe400eed84272703c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc1, 0xb7, 0xd8 }; // Pissedoffcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PissedoffcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PissedoffcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PissedoffcoinMiner(CWallet *pwallet)
{
    printf("PissedoffcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pissedoffcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PissedoffcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PissedoffcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PissedoffcoinMiner, pwallet));
}
